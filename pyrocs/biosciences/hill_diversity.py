...
def hill_shannon(p: np.array) -> float:
    """
    The Hill-Shannon number is a specific instance (i.e. the Perplexity) of Hill Diversity, which prioritizes neither common nor rare species. The use of the geometric mean captures the proportional difference from the mean of extreme values (rather than the absolute values). The equation for the Hill-Simpson based on the formulation in [Roswell et al., 2021](https://doi.org/10.1111/oik.07202):
    
    Hill Shannon (Perplexity): H_q=e^(-sum(p_i*ln(p_i))
...
def hill_simpson(p: np.array) -> float:
    """
    The Hill-Simpson number is a specific instance (i.e. the Inverse Simpson Index) of Hill Diversity that prioritizes the common species. The use of an arithmetic mean gives more weight to more frequently occurring species. The equation for the Hill-Simpson based on the formulation in [Roswell et al., 2021](https://doi.org/10.1111/oik.07202):
    
    Hill Simpson (Inverse Simpson Index): H_2 = 1/Sum[p_i^2]
...
def hill_diversity(p: np.array, q: float) -> float:
    """
    The Hill Numbers are a family of diversity metrics describing "effective number of species". For intuition, consider a distribution over N species, but only K of them have a "significant" share of the distribution, the remaining species together having a "small" share of the distribution. The simple count of number of species, N, is sensitive to the presence of individuals from rare species. However, a formula which somehow discards or discounts rare species, returning a value close to K, is more robust and better reflects the number of important species.
    ...
    The equations for the set of Hill metrics are based on the formulation in [Roswell et al., 2021](https://doi.org/10.1111/oik.07202)
	Hill Diversity: H_q = (Sum[p_i^q])^(1/(1-q))
